...
/**
  * In-memory token bucket keyed by an opaque string. Map size is bounded
  * by `maxKeys`; when the cap is reached the map is cleared (crude but
  * bounded under a hostile fanout).
  */
